...
def draw_point(surface, color, position, radius=3):
...
class Object3D:
    def __init__(self, position, rotation=Angle3(0, 0, 0)):
...
class Camera:
    ...
    def __init__(self, world, position, rotation, size):
        self.world = world
        self.position = position
        self.rotation = rotation
        self.size = size
        ...
        self.v_fov = self.h_fov * (self.h / self.w)
        self.focal_length = self.half_w / math.tan(self.h_fov / 2)
    ...
    def render_object(self, surface, obj):
        if isinstance(obj, ObjectGroup):
            ...
            lst_pos2d_light = [self.convert_to_2d(self.position, pos, get_light=True)
                               for pos in points]
            dist = self.position.distance_squared_to(obj.position)
            ...
            lst_pos2d_light = [self.convert_to_2d(self.position, pos, get_light=True) for pos in obj.vertexes]
            ...
            res_pos_light = self.convert_to_2d(self.position, obj.position, get_light=True)
            if res_pos_light:
                dist = self.player.position.distance_squared_to(obj.position)
    ...
    def convert_to_2d(self, my_pos: Vector3, obj_pos: Vector3, get_light=False):
        ...
        angle_z = self.rotation.z
        vec_to = (obj_pos - my_pos)
        vec_to.rotate_z_rad_ip(angle_z)
        vec_to.rotate_x_rad_ip(self.rotation.x)
        dist_y = vec_to.y
    ...
    def draw_sky(self, surface):
        y = self.half_h - math.tan(self.rotation.x) * self.focal_length
        # if y < 0:
        #     surface.fill()
        if y > self.h:
...
def main():
    ...
    sceleton = SkeletonRotate(*open_file_obj("monkey1.obj", 7, _convert_faces_to_lines=True),
